...
export const boutiqueAPI = {
  ...
  getProducts: () => api.get('/boutique/products').then(getData),
  createProduct: (payload: any) => api.post('/boutique/products', payload).then(getData),
  updateProduct: (id: string, payload: any) => api.put(`/boutique/products/${id}`, payload).then(getData),
  deleteProduct: (id: string) => api.delete(`/boutique/products/${id}`).then(getData),
