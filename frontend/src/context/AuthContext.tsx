...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Helper to extract the most useful error message from an Axios error
  ...
  const googleLoginSim = async (name: string, email: string) => {
    ...
    try {
      ...
      const res = await authAPI.googleLogin({ name, email, googleId });
      const sessionData = res.data || res;
      saveSession(sessionData.token, sessionData.user);
    } catch (error: any) {
      clearSession();
      throw extractErrorMessage(error, 'Google authentication failed. Please try again.');
    } finally {
      setLoading(false);
    }
  };
